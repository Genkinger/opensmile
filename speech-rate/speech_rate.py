...
def result_to_csv(result):
    ...
    for k,v in result.items():
        output_csv = output_csv + k + ',' + str(v["file_length_in_seconds"]) + ',' + str(v["word_count"]) + ',' + str(v["words_per_second"]) + ',' + str(v["words_per_minute"]) + '\n'
    return output_csv
...
def main():
    res = count_words_per_file("pfad/zur/transcript.txt","./pfad/zu/den/audio/dateien") #NOTE(L): Hier muessen noch die richtigen Werte eingetragen werden!
